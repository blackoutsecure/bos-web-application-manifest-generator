...
Use favicon mode for a quick setup with default favicon settings. This mode supports any favicon format and provides sensible defaults based on common favicon generator outputs (e.g., Android Chrome icons, Apple touch icons, etc.).
...
**Note:** The default icons reference Android Chrome format (`/android-chrome-192x192.png` and `/android-chrome-512x512.png`), which is commonly generated by favicon tools. You can change the path prefix using `favicon_dir` or override them entirely using `favicon_options`.

### Favicons Mode with Custom Options

Override specific defaults using `favicon_options`. You can customize any manifest property including app name, icons, colors, and more:
...
| `favicon`                 | Enable favicon mode (supports any favicon format) (default: true)           | `true`                                  |
| `favicon_dir`             | Directory path prefix for default favicon icons (default: /)                | `/`                                     |
| `favicon_validation`      | Favicon file validation mode: `fail`, `warn`, or `none` (default: warn)     | `warn`                                  |
| `favicon_options`         | JSON object to override favicon defaults                                    | -                                       |
...
| `icons`                   | JSON array of icon objects (not required if favicon enabled)                | Android Chrome icons (192x192, 512x512) |
| `filename`                | Output manifest filename                                                    | `site.webmanifest`                      |
| `inject_manifest_link`    | Auto-inject link tag into HTML pages                                        | `true`                                  |
| `icon_validation`         | Icon validation for standard icons: `fail`, `warn`, or `none`               | `warn`                                  |
...
When `favicon: true` (default, favicon mode):
...
When `favicon: false` (standard mode):

- Same defaults as favicon mode, except favicon files are not validated
- Icon validation uses `icon_validation` input instead

You can override any of these by explicitly setting the input values in your workflow.

## Favicon Mode

Favicon mode provides a quick way to generate a standard web manifest without manually specifying all properties. When enabled (`favicon: true`, which is the default), it uses sensible defaults based on common favicon generator outputs.
...
- **Favicon Dir**: `/` (default path prefix for icons - change with `favicon_dir` input)
- **Theme Color**: `#ffffff`
- **Background Color**: `#ffffff`
- **Display**: `standalone`
- **Name/Short Name**: Empty strings (override via `favicon_options` or top-level inputs)
- **Favicon Validation**: Warn on missing files (set `favicon_validation: fail` to fail action, `none` to skip)

### Customizing Favicon Directory

If your favicon files are in a subdirectory, use `favicon_dir`:
...
### Overriding Favicon Defaults

You can override any defaults using the `favicon_options` parameter:
...
All standard manifest properties can be overridden through `favicon_options`.

### When to Use Each Mode

**Use Favicon Mode** (`favicon: true`, which is the **default**) when:
...
- You want automatic validation that favicon files exist
- You want to override just a few properties via `favicon_options`

**Use Standard Mode** (`favicon: false`, `icons: [...]`) when:
...
- You don't want automatic favicon validation

## Icon Validation

### Favicon Mode Validation

When `favicon: true` (default), the action automatically validates that favicon files exist:

- **`favicon_validation: 'fail'`** - Fails the action if any favicon files are missing
- **`favicon_validation: 'warn'`** - Warns about missing icons but continues (default)
- **`favicon_validation: 'none'`** - Skips favicon validation
...
- name: Generate Web Manifest
  ...
  with:
    public_dir: 'dist'
    favicon: true
    # favicon_validation: 'warn' is the default
```
...
- name: Generate Web Manifest
  ...
  with:
    public_dir: 'dist'
    favicon: true
    favicon_validation: 'fail' # Fails if favicons missing
```

### Standard Mode Validation

When `favicon: false` (standard mode), the action validates custom icons:

- **`icon_validation: 'fail'`** - Fails the action if any icons are missing
- **`icon_validation: 'warn'`** - Warns about missing icons but continues (default)
- **`icon_validation: 'none'`** - Skips validation
